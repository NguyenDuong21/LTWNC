...
!function(a) {
	...
	function f() {
		var a                                                                       = {
			...
			}, b = new google.maps.Map(document.getElementById("map-canvas"), a), c = "images/map-marker.png",
	...
	function l() {
		...
		if(a("#cartItemsType2").length) {
			for(var c = 0; F > c; c++) {
				b += m(E[c].image, E[c].name, E[c].price, E[c].quantity, E[c].id, 2);
			}
	...
	function m(a, b, c, d, e, f) {
		for(var g = "", h = 1; 5 > h; h++) {
			g += '<option value="' + h + '"' + (d === h ? "selected" : "") + ">" + h + " product</option>";
		}
	...
	function o(a) {
		var b                      = new Date, c = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"],
			d                      = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"],
			e = b.getFullYear(), f = c[b.getDay()] + "  " + b.getDate() + d[b.getMonth()] + " " + e;
		a.html(f)
	}
	...
	function r() {
		if(a(window).width() > 750) {
			...
			}, {offset: "40%"}), r.find(".pricing-table:nth-of-type(3)").waypoint(function() {
				r.find(".pricing-table:nth-of-type(3)").addClass("animated fadeInRight")
			}, {offset: "40%"}), r.find(".pricing-table:nth-of-type(4)").waypoint(function() {
				r.find(".pricing-table:nth-of-type(4)").addClass("animated fadeInLeft")
			}, {offset: "40%"}), r.find(".pricing-table:nth-of-type(5)").waypoint(function() {
				r.find(".pricing-table:nth-of-type(5)").addClass("animated fadeInRight")
			}, {offset: "40%"}))
		}
	}

	var s, t = !1, u = 200, v = a(window), w = v.width();
	...
	x.isotope({itemSelector: ".grid-item"}), a(".categories  li button").on("click", function(b) {
		b.preventDefault();
	...
	var E = JSON.parse(sessionStorage.getItem("cartItems")) || [], F = E.length, G = 0;
	if(g(), l(), k(), a(".addToCart").on("click", function(b) {
		...
		b.preventDefault();
		var c = JSON.parse(sessionStorage.getItem("cartItems")), d = JSON.stringify(c);
		...
		var H = a(".event-timer");
		H.each(function() {
			var b = a(this), c = b.attr("data-date");
	...
	a("a.back-to-top").on("click", function(b) {
		a("html, body").stop().animate({scrollTop: a("body").offset().top}, 1500), b.preventDefault()
